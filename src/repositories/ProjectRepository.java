...
public class ProjectRepository {
    ...
    public List<Project> getAll() {
        ...
        try (Connection connection = Database.getInstance().getConnection(); PreparedStatement preparedStatement = connection.prepareStatement(q)) {
            ...
            while (resultSet.next()) {
                ...
                projects.add(new Project(projectId, name, profitMargin, costTotal, Status.valueOf(status), null, null, null, null));
            }
            return projects;
        } catch (SQLException e) {
            throw new RuntimeException(e);
    ...
    public Project editProject(Project project) {
        String q = "UPDATE projects SET name = ? , profit_margin = ? , cost_total = ? WHERE id = ?";
        try (Connection connection = Database.getInstance().getConnection(); PreparedStatement preparedStatement = connection.prepareStatement(q)) {
            preparedStatement.setString(1, project.getName());
            preparedStatement.setDouble(2, project.getProfitMargin());
            preparedStatement.setDouble(3, project.getCostTotal());
    ...
    public boolean deleteProject(Project project) {
        String q = "DELETE FROM projects WHERE id = ?";
        try (Connection connection = Database.getInstance().getConnection(); PreparedStatement preparedStatement = connection.prepareStatement(q)) {
